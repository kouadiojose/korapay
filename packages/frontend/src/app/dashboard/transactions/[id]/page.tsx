...
export default function TransactionDetailPage() {
  ...
  const txn = getTransaction(params.id as string);

  if (!txn) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <h2 className="text-xl font-semibold text-gray-900 mb-2">Transaction introuvable</h2>
        <p className="text-gray-500 mb-6">Cette transaction n&apos;existe pas ou a été supprimée.</p>
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-8">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{formatCurrency(txn.amount, txn.currency)}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Frais</p>
                <p className="text-sm font-medium text-gray-900">{formatCurrency(txn.fee, txn.currency)}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Montant net</p>
                <p className="text-sm font-semibold text-gray-900">{formatCurrency(txn.net_amount, txn.currency)}</p>
              </div>
              <div>
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
              <div>
                ...
                <p className="text-sm font-medium text-gray-900">{txn.provider}</p>
              </div>
              {txn.provider_reference && (
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => copyToClipboard(txn.provider_reference!)}
                      className="text-gray-400 hover:text-gray-600"
                    >
                      <Copy size={14} />
                    </button>
                ...
                return (
                  <div key={i} className="flex gap-4">
                    <div className="relative">
                      ...
                      {i < getTimelineForStatus().length - 1 && (
